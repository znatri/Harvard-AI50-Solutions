...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for variable in self.domains:
            for value in set(self.domains[variable]):
    ...
    def ac3(self, arcs=None):
        ...
        x, y = arcs.pop()
        if self.revise(x, y):
            if self.domains[x] is None or self.domains[y] is None:
                return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.domains.keys():
            if var not in assignment:
                ...
                if len(self.domains[var]) < len(self.domains[current]):
                    current = var

                elif len(self.crossword.neighbors(var)) > len(self.crossword.neighbors(current)):
                    current = var
